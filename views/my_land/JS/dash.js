vm_landsearch = new Vue({
	...
	data: {
		...
		current_page: 1,
		pages: [],
		api_land	:{'ADV_NO':"",'TYPE':1,"FOR":"RENT_Y","INTERFACE":"N","location":[]},
		upd_land	:{'ADV_NO':"",'TYPE':1,"FOR":"RENT_Y","INTERFACE":"N",},
	...
	methods: {
		...
		get_api_land(){
			var x = this;
			var cu = $('#new_api_form').attr('action');
			
			$("#kk_loader").removeClass("d-none");
			$.post(cu,$('#new_api_form').serializeArray(),function(data,status,xhr){
				try {
				    ...
				    $("#kk_loader").addClass("d-none");
				    set_location_map('map_area','new_location','ADV','new_lat','new_lng',JSON.stringify(x.api_land.LOCATION_A) );
				    //set_location_map('map_area','new_map_data','ADV','','');
				    $('#new_land_api').modal('toggle');
				    $("#new_land").modal('toggle');
