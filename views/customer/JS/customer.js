vm_staff = new Vue({
	...
	methods: {
		...
		update_staff(index){
			$('#upd_index').val(index);
			$('#upd_id').val(this.owner[index]['ID']);
			$('#upd_name').val(this.owner[index]['NAME']);
			$('#upd_phone').val(this.owner[index]['PHONE']);
			$('#upd_email').val(this.owner[index]['EMAIL']);
			$('#upd_nat_no').val(this.owner[index]['NAT_NO']);
		},
		active(index){
			
			var crt= $("#csrf").val();
			var id = this.owner[index]['ID'];
			
			var active = this.owner[index]['ACTIVE'] == 1;
			var lib = this.owner;
			$.post(URL+"customer/active",{csrf:crt,id:id,current:active},function(data,status,xhr){
				try {
