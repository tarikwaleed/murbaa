                ...
                <div class="container p-1 ">
                    <div class="row">
						<div class="col-sm-8">
							...
							<hr/>
							<div class="row border m-0" v-if="config.ID != SER.CO_ID && !config.SER_REG_NO">
								...
								<div v-for="(x, index) in SER.OFFERS" class="col-md-11 m-0 property-item g-0 bg-hover-light rounded-lg overflow-hidden flex-md-row mb-2 shadow-sm position-relative">
									<div class="row">
										...
										<div class="col-2">
											...
											<button v-if="config.ID == x.CO_ID && SER.STATUS == 'NEW'" type="button" class="btn btn-block btn-warning" data-toggle="collapse" :data-target="'#collapseExample_'+index" aria-expanded="false" aria-controls="collapseExample">تعديل <i class="fa fa-edit"></i></button>
										</div>
									</div>
									<div class="row " v-html="x.D_DESCR"></div>
									<div class="row" v-if="x.FILES && (config.ID == x.CO_ID || config.ID == SER.CO_ID)">
