	...
	class dashboard_model extends model
	{
		...
		public function search()
		{
			...
			$lands =  $this->db->select("SELECT l_id, l_block, l_loc_lat, l_loc_lng
										...
										,l_co_relation, l_co_delegat, l_like
			...
			foreach($lands as $val)
			{
				...
				//get like and visit
				$sing['IS_LIKE'] = cookies::get('land_like_'.$val['l_id']);
		...
		public function land_like($id)
		{
			...
			$land =  $this->db->select("SELECT l_id, l_like FROM ".DB_PREFEX."land
										WHERE l_id = :ID
									" ,array(":ID"=>$id));
			if(count($land) != 1)
			{
			...
			$land = $land[0];
			if(cookies::get('land_like_'.$land['l_id']))
			{
				return "تم تسجيل اعجابك بهذا العقار من قبل";
			}
			
			//new like
			cookies::set('land_like_'.$land['l_id'],$land['l_id']);
			$like_no = $land['l_like'] + 1;
			$this->db->update(DB_PREFEX.'land',array('l_like'=>$like_no),'l_id = '.$land['l_id']);
