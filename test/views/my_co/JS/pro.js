vm_staff = new Vue({
	...
	data: {
		...
		types		: js_types,
		pkg			: js_package,
		ser_reg_type: js_ser_type,
		curr_reg_type: "",
		total		: 0,
    },
	mounted(){
		if(this.info.SER_REG_TYPE != '' && this.info.SER_REG_TYPE != null)
		{
			this.curr_reg_type = this.info.SER_REG_TYPE;
		}else
		{
			this.curr_reg_type = "MARKET";
		}
...
function modal_element($type,$id)
{
	switch($type)
	{
		...
		case "new_reg":
		case "upd_ser":
			const myTimeout = setTimeout(function(){location.reload(); }, 10000);
		break;
	}
}
