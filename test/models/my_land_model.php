<?php
	...
	class my_land_model extends model
	{
		...
		public function new_api_land()
		{
			...
			//Get AD_LINC
			$co = $this->db->select("SELECT co_id AS ID, co_name AS NAME
			...
			if(empty($reg))
			{
			    $region = array("reg_id"=>$result['location']['regionCode']
			                    ,"reg_name"=>$result['location']['region']
			                    ,"reg_name_en"=>$result['location']['region']
			                    ,"create_at"=>$time);
			...
			if(empty($city))
			{
			    $city = array("c_id"=>$result['location']['cityCode']
			                    ,"c_name"=>$result['location']['city']
			                    ,"c_name_en"=>$result['location']['city']
			                    ,"c_region"=>$result['location']['regionCode']
			                    ,"create_at"=>$time);
			...
			if(empty($nei))
			{
			    $nei = array("nei_id"=>$result['location']['districtCode']
			                    ,"nei_name"=>$result['location']['district']
			                    ,"nei_name_EN"=>$result['location']['district']
			                    ,"nei_city"=>$result['location']['cityCode']
			                    ,"create_at"=>$time);
			...
			if(empty($ty))
			{
			    $ty = array("ty_name"=>$result['propertyType']
			                    ,"ty_name_en"=>$result['propertyType']
			                    ,"ty_type"=>"HUM"
			                    ,"create_at"=>$time);
		...
		public function new_land()
		{
			...
			$area_info = $this->location_info($fdata['new_lat'],$fdata['new_lng']);
			...
			if($fdata['new_relation'] == "SAIL" ||$fdata['new_relation'] == "CREA")
			{
				...
				{
					return array('Error'=>"In Field new_delegate : عليك ان تحدد رقم التفويض او ملف التفويض.. \n ");
				}
			}
			if(count($cus)!= 1)
			{
			...
			//if(empty($cus[0]['comm_co_num']) || (empty($cus[0]['comm_real_no']) && ($fdata['new_relation'] == "SAIL" ||$fdata['new_relation'] == "CREA")))
			if(empty($cus[0]['comm_real_no']) && ($fdata['new_relation'] == "SAIL" ||$fdata['new_relation'] == "CREA"))
            {
