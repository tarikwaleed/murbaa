	...
	class my_land_model extends model
	{
		...
		public function conf_list()
		{
			...
			if(count($pk) == 1)
			{
				...
				$ret['ACCEPT']	= $pk[0]['comm_accept'];
				$ret['ID_TYPE']	= $pk[0]['co_id_type'];
				$ret['REAL_NO']	= $pk[0]['comm_real_no'];
		...
		public function search()
		{
			...
			foreach($lands as $val)
			{
				...
				$comp = $this->db->select("SELECT adv_co AS ID FROM ".DB_PREFEX."land_adv
														WHERE adv_land = :LAND"
													,array(':LAND'=>$val["l_id"]));
				$sing['COMPANY'] = array();
				foreach($comp as $v)
		...
		{
			$form	= new form();
			$form	->post('new_block') 	// BLOCK
					->valid('numeric')
					
					//->post('new_lng') 		// LOCATION
					//->valid('numeric')
					
					//->post('new_lat') 		// LOCATION
					//->valid('numeric')
					
					->post('new_map_data') 		// LOCATION
					//->valid('Min_Length',5)
					...
					->post('new_unit_no',false,true)	 // UNit_no
					->valid('numeric')
					
					->post('new_unit_nun',false,true)	 // unit_num
					->valid('numeric')
					...
					->post('new_air_cond',false,true)
					->valid('numeric')
					
					->post('new_mortage',false,true) 	// الرهن
					->valid('Min_Length',5)
					
					->post('new_law',false,true) 	// الحقوق والالترامات
					->valid('Min_Length',5)
					
					->post('new_info',false,true) 	// المعلومات التي تؤثر على العقار
					->valid('Min_Length',5)
					
					->post('new_dispute',false,true) 	// النزاعات
					->valid('Min_Length',5)
					
					->post('new_des_n',false,true)
					->valid('numeric')
					
					->post('new_des_e',false,true)
					->valid('numeric')
					
					->post('new_des_w',false,true)
					->valid('numeric')
					
					->post('new_des_s',false,true)
					->valid('numeric')
			...
			$fdata['new_map_data'] = $_POST['new_map_data'];
			
            $f_location = str_replace('quot;','"',$fdata['new_map_data']);
			...
			$cus = $this->db->select("SELECT comm_id, comm_exp_date, comm_accept, comm_no
										,comm_real_no, comm_co_num, co_id_type, co_id_no
										FROM ".DB_PREFEX."comm_reg AS co_reg
										JOIN ".DB_PREFEX."company ON comm_co = co_id
										WHERE comm_co = :ID AND 
										now() BETWEEN DATE(co_reg.create_at) AND comm_exp_date
										AND comm_accept = 1
									" ,array(":ID"=> session::get('company')));
			...
			/*if(empty($cus[0]['comm_no']) || (empty($cus[0]['comm_real_no']) && ($fdata['new_relation'] == "SAIL" ||$fdata['new_relation'] == "CREA")))
			{
			...
			if($fdata['new_relation'] == "SAIL" ||$fdata['new_relation'] == "CREA")
			{
                ...
                if(empty($fdata['new_delegate']) && empty($_FILES['new_delegate_file']))
				{
			    ...
			    $type 	= $my_co['co_id_type']; //From Types { 1,2,3}
			    $id_no 	= $my_co['co_id_no']; //National no - iqama, ....
			    $auth_no= $fdata['new_delegate']; // delegated auth no
			    $adv_no = $my_co['comm_no']; // delegated adv no
			...
			$land['l_for'] 			= $fdata['new_for'];
			$land['l_price'] 		= $fdata['new_price'];
			$land['l_currency'] 	= $fdata['new_currency'];
			$land['create_at'] 		= $time;
			$land['create_by'] 		= session::get('user_id');
			$land['l_location'] 	= (!empty($fdata['new_map_data']))?$fdata['new_map_data']:null;
			$land['l_corner'] 		= (!empty($fdata['new_corner']))?$fdata['new_corner']:null;
			$land['l_road'] 		= (!empty($fdata['new_road']))?$fdata['new_road']:null;
			
			$land['l_mortgage'] 	= (!empty($fdata['new_mortage']) && $fdata['new_mortage']!= 'لا يوجد' )?$fdata['new_mortage']:null;
			$land['l_law'] 			= (!empty($fdata['new_law']) && $fdata['new_law']!= 'لا يوجد' )?$fdata['new_law']:null;
			$land['l_info'] 		= (!empty($fdata['new_info']) && $fdata['new_info']!= 'لا يوجد' )?$fdata['new_info']:null;
			$land['l_disputes'] 	= (!empty($fdata['new_dispute']) && $fdata['new_dispute']!= 'لا يوجد' )?$fdata['new_dispute']:null;
			$land['l_expered'] 		= $exp;
			
			$land['l_interface'] 	= (!empty($fdata['new_interface']))?$fdata['new_interface']:null;
					
			$land['l_des_n'] 		= (!empty($fdata['new_des_n']))?$fdata['new_des_n']:null;
			$land['l_des_e'] 		= (!empty($fdata['new_des_e']))?$fdata['new_des_e']:null;
			$land['l_des_w'] 		= (!empty($fdata['new_des_w']))?$fdata['new_des_w']:null;
			$land['l_des_s'] 		= (!empty($fdata['new_des_s']))?$fdata['new_des_s']:null;
			...
			switch($ty['BUILD'])
			{
				case 1: //building
					...
					$land['l_unit_no'] 		= (!empty($fdata['new_unit_no']))?$fdata['new_unit_no']:1;
					$land['l_unit_num'] 	= (!empty($fdata['new_unit_nun']))?$fdata['new_unit_nun']:1;
					$land['l_condition'] 	= (!empty($fdata['new_air_cond']))?$fdata['new_air_cond']:null;
		...
		public function upd_land()
		{
			...
			if(count($cus)!= 1)
			{
				return array('Error'=>"In Field upd_relation : عليك ان تقوم بتوثيق حسابك اولا.. \n ");
			}
			/*if(empty($cus[0]['comm_no']) || (empty($cus[0]['comm_real_no']) && ($fdata['upd_relation'] == "SAIL" ||$fdata['upd_relation'] == "CREA")))
			{
				return array('Error'=>"In Field upd_relation : عليك ان تضيف رقم رقم ترخيص الهيئة العامة للعقار ورقم المعلن لتوثيق حسابك.. \n ");
			}*/
			if($fdata['upd_relation'] == "SAIL" ||$fdata['upd_relation'] == "CREA")
			{
				if(empty($fdata['upd_delegate']) && empty($_FILES['upd_delegate_file']))
				{
					return array('Error'=>"In Field upd_delegate : عليك ان تحدد رقم التفويض.. \n ");
				}
                if(!empty($fdata['upd_delegate']))
                {
                ////////////////////////////////////////////////////////////////////////////////////////////
                $my_co = $cus[0];
                //Get Tocken:
			    ...
			    $type 	= $my_co['co_id_type']; //From Types { 1,2,3}
			    $id_no 	= $my_co['co_id_no']; //National no - iqama, ....
			    $auth_no= $fdata['upd_delegate']; // delegated auth no
			    $adv_no = $my_co['comm_no']; // delegated adv no
		...
		public function upgrade_vip()
		{
			...
			$pay_ok = payments::pay($this->db,$fdata['token'],
									$fdata['vip_price'],
									$pay_code,"land Payment",'my_land/ret/');
			...
			{
				$bill['bi_status']= "PEND";
				$bill['bi_ins_data']= json_encode(array('table'=>DB_PREFEX.'company'
														,'data'=>$arr));
		...
		private function location_info($lat,$lng)
		{
			...
			//check country
			$cou = $this->db->select("SELECT cou_id FROM ".DB_PREFEX."country
									WHERE cou_code = :COU "
			...
			//check city
			$cou = $this->db->select("SELECT c_id FROM ".DB_PREFEX."city
									WHERE (c_name like :NAM OR c_name_EN like :NAM2) AND c_country = :COU"
									,array(":NAM"=>$location_data['short']["administrative_area_level_2"]
											,":NAM2"=>$location_data['short']["administrative_area_level_2"]
											,":COU"=>$country));
			if(count($cou) != 1)
			{
				//city not found
			...
			$cou = $this->db->select("SELECT nei_id  
										FROM ".DB_PREFEX."neighborhood
