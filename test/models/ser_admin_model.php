	...
	class ser_admin_model extends model
	{
		...
		public function services()
		{
			...
			$xx = $this->db->select("SELECT ser_id AS ID, ser_type AS TYPE, ser_price_from AS PRICE_FROM
									...
									,ser_city AS CITY, ser_sel_type AS SEL_TYPE
									
									,ser_co AS CO_ID, OWN.co_name AS CO_NAME, OWN.co_img AS CO_IMG
									
									,count(OFFERS.off_id) AS OFF_NO
									
									,ser_offer AS OFFER_ID, CURR_OFFERS.off_price AS OFFER_PRICE
									,CURR_OFFERS.off_percentage AS OFFER_PER, CURR_OFFERS.off_period AS OFFER_PERIOD
									,CURR_OFFERS.off_desc AS OFFER_DESC, CURR_OFFERS.off_co AS OFF_CO_ID
									,OFF_OWN.co_name AS OFF_CO_NAME, OFF_OWN.co_img AS OFF_IMG
									
									FROM ".DB_PREFEX."service AS SER
									JOIN ".DB_PREFEX."company AS OWN ON OWN.co_id = ser_co
									LEFT JOIN ".DB_PREFEX."service_offer AS OFFERS ON OFFERS.off_service = ser_id 
									LEFT JOIN ".DB_PREFEX."service_offer AS CURR_OFFERS ON CURR_OFFERS.off_id = ser_offer 
									LEFT JOIN ".DB_PREFEX."company AS OFF_OWN ON OFF_OWN.co_id = CURR_OFFERS.off_co
